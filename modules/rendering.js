...
function updateBuildListDisplay(unitId, forceSync = false, renderLimit = 150) {
    ...
    const renderListInternal = (builds, limit) => {
        ...
        if (prioSelect === 'all' && hasActiveFilters) {
            const uniqueMap = new Map();
            filtered.forEach(r => {
                const getW = (x) => {
                    if (unitId === 'sjw' && x.headUsed === 'sun_god') return 1.05;
                    if (unitId.includes('sasuke') && x.headUsed === 'biju_head') return 1.2;
                    return 1.0;
                };
                const weight = getW(r);
                const key = r.setName + r.traitName + (r.headUsed || 'none');
                if (!uniqueMap.has(key) || (r.dps * weight > uniqueMap.get(key).dps * getW(uniqueMap.get(key)))) {
                    uniqueMap.set(key, r);
                }
            });
    ...
    if (!buildData && !inventoryMode && unitObj) {
        ...
        if (!isBambiAlt && !isRobotAlt) {
            ...
            buildData = window.unitBuildsCache[unitId]?.[activeType]?.[activeMode]?.[activeCfg];
        }
    }
        ...
        for (let i = 0; i < 4; i++) {
           ...
           if (loadedFromStatic && anyGlobalBuff && typeof reconstructMathData === 'function') {
                ...
                calculatedResults.forEach((res, index) => {
                    ...
                    if (index < 30 || traitCounts[tName] < 5) { subset.push(res); traitCounts[tName]++; }
                });
...
// --- ADD THIS BLOCK FOR CELL ---
    ...
    `;
            ...
            const bottomControls = `
                <div class="search-container">
                    ...
                    <div class="search-row">
                        ...
                        <select onchange="filterList(this)" data-filter="head" class="search-select"><option value="all">All Heads</option><option value="sun_god">Sun God</option><option value="ninja">Ninja</option><option value="reaper_necklace">Reaper</option><option value="shadow_reaper_necklace">Shadow Reaper</option><option value="junior">Junior Ninja</option><option value="biju_head">Biju</option><option value="none">No Head</option></select>
                    </div>
                </div>`;
            ...
            const card = createBaseUnitCard(unit, {
                ...
                bannerContent: `<div class="placement-badge">Max Place: ${unit.placement}</div>${getUnitImgHtml(unit, 'unit-avatar')}<div class="unit-title"><h2>${unit.name}</h2><span>${unit.role}</span></div>${unit.meta ? `<button class="trait-guide-btn" onclick="openTraitGuide('${unit.id}')">📋 Rec. Traits</button>` : ''}`,
