...
function renderSourceTotalsSection(data) {
   ...
   const passiveTotalSpa = (data.passiveSpaBuff || 0) + (data.enlightSpa || 0) + (data.bijuuSpa || 0) + (data.kingMarkSpa || 0);
    const passiveTotalRange = (data.baseStats.passiveRange || 0) + (data.eternalRangeBuff || 0) + (data.enlightBuff || 0) + (data.bijuuBuff || 0);
    const passiveTotalCrit = (data.amCritRate || 0) + (data.ksCrit || 0);
            ...
            <div style="border-left: 3px solid #fbbf24; padding-left: 10px;">
                <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 6px;">
                    ...
                    <div style="text-align: right; display: flex; flex-direction: column; align-items: flex-end;">
                        ...
                        ${data.totalHyper > 0 ? `<div style="font-size: 0.65rem; color: #ef4444;">+ ${data.totalHyper}% Hyper</div>` : ''}
                    </div>
                </div>
...
function renderActiveBuffsSection(data) {
    ...
    if (data.ksCrit > 0) buffs.push({ name: "Ancient Mage", desc: "Active: +20% Crit Rate/Dmg, 30s Follow-up", color: "#60a5fa" });
...
function renderMathContent(data) {
    ...
    } else if (data.headBuffs && data.headBuffs.type === 'biju') {
        const uptimePct = (data.headBuffs.uptime || 0);
        headDmgHtml = `
        <tr class="mt-row-sungod" style="border-color: rgba(248, 113, 113, 0.4);"><td colspan="3" class="p-2">
            <div class="mt-flex-between mb-2"><span style="color: #f87171" class="mt-text-bold text-xs tracking-sm">BIJU HEAD PASSIVE (SASUKE)</span><button class="calc-info-btn" onclick="openInfoPopup('sungod_passive')">?</button></div>
            
            <div class="mt-flex-between text-xs text-white mb-1">
                <span class="opacity-70">Trigger (3 Atks):</span>
                <span class="mt-font-mono mt-text-right text-white">${fmt.fix(data.headBuffs.trigger, 1)}s</span>
            </div>
            <div class="mt-flex-between text-xs text-white mb-1">
            ...
            <div class="mt-flex-between text-xs text-white mb-3">
                <span class="opacity-70">Avg. Uptime:</span>
                <span class="mt-font-mono mt-text-right ${uptimePct >= 1 ? 'mt-text-green' : 'mt-text-orange'}">${fmt.fix(uptimePct*100,1)}%</span>
            </div>

            <div class="mt-flex-between mt-border-top mt-pt-sm"><span class="text-white text-xs text-bold">Avg Damage Buff</span><span style="color: #f87171" class="text-sm mt-text-bold"> +${fmt.num(data.headBuffs.dmg)}%</span></div>
        </td></tr>`;
    } else if (data.headBuffs && data.headBuffs.type === 'biju') {
        const uptimePct = (data.headBuffs.uptime || 0);
        headDmgHtml = `
    ...
    return `
        ...
        <div class="breakdown-top-wrapper" style="position:relative; margin-bottom: 12px;">
            ...
            <button class="breakdown-swap-btn" onclick="swapBreakdownPanels(this)" title="Toggle panel view" aria-label="Swap panels">
                <span style="display:inline-flex;align-items:center;gap:4px;">&#8644; <span class="swap-label">Details</span></span>
            </button>
        </div>
        ${renderQuickBreakdownSection(data, avgHitPerUnit, dotColorClass)}
        ${renderActiveBuffsSection(data)}
