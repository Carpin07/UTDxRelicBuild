...
const getBestSubConfig = (build, stats, includeSubs, headMode, candidates, optimizeFor = 'dps') => {
    ...
    let headOptions = (mode === 'auto') 
        ? ['sun_god', 'ninja', 'reaper_necklace', 'shadow_reaper_necklace', 'biju_head'] 
...
function _calcHeadDynamicBuffs(headPiece, finalSpa, finalRange, uStats) {
    ...
    if (headPiece === 'sun_god') {
        ...
        headDotBuff = finalRange * 0.75; headCalc.type = 'reanimated';
...
function calculateDPS(uStats, relicStats, context) {
    ...
    const setAndPassiveSpa = (sBonus.spa || 0) + passiveSpaPcent + enlightSpa + bijuuSpa + kmSpa;
    ...
    // True Damage (Hyper) is currently disabled (non-functional in-game)
    const totalHyper = 0; 
    let additiveTotal = (sBonus.dmg || 0) + passivePcent + headDmgBuff + mikuBuff + enlightBuff + bijuuBuff + totalHyper + kmDmg;

    // Junior Ninja: 1.1x Multiplier to Miku Buff and Passives (WATER GOD ONLY)
    if (headPiece === 'junior' && uStats.id === 'water_god') {
        additiveTotal = ((sBonus.dmg || 0) + passivePcent + headDmgBuff + mikuBuff + totalHyper + enlightBuff + bijuuBuff + kmDmg) * 1.1;
    }
    ...
    const finalCritRate = Math.min(uStats.crit + traitCritRate + amCritRate + ksCrit + ((uStats.id === 'kirito') ? 0 : (baseR_Cf + (sBonus.cf || 0))), 100);
    ...
    if (amSupportActive) {
        const amMult = (1 + usedSpa / 30);
        attackMultiplier *= amMult;
        if (!extraAttacksData) {
            extraAttacksData = { hits: "30s Follow-up", mult: amMult, label: "Ancient Mage" };
        } else {
            extraAttacksData.label = (extraAttacksData.label || "Attack Rate") + " + Ancient Mage";
            extraAttacksData.mult *= amMult;
        }
    }
   ...
   return {
        ...
        tagBuffs,
        mikuBuff: mikuBuff,
        totalHyper: totalHyper,
        hyperMult: 1 + (totalHyper / 100),
